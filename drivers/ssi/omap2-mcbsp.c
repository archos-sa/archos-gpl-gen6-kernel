...
static struct omap_dma_channel_params rx_params;
...
static void  mcbsp_power_settings(u32 mcbsp_id, int level)
{
	...
	if(level == MCBSP2_SYSCONFIG_LVL1)
		...
		CLOCKACTIVITY(MCBSP_SYSC_IOFF_FON) | SIDLEMODE(NO_IDLE));
...
int omap2_mcbsp_txmultich_enable(u32 mcbsp_id, u32 state)
{
	...
	if (state < OMAP2_MCBSP_TXMUTICH_ENABLE_NOMASK ||
	    state > OMAP2_MCBSP_TXMUTICH_DISABLE_SYMETRIC) {
		D2(KERN_ERR
		   "OMAP2_McBSP: Invalid txmultichannel enable state \n");
		up(&omap2_mcbsp_info[mcbsp_id].semlock);
		FN_OUT(-EINVAL);
		return -EINVAL;
	} else {
		OMAP2_MCBSP_WRITE(base, MCR2,
				  OMAP2_MCBSP_READ(base, MCR2) | (XMCM(state)));
	}
	up(&omap2_mcbsp_info[mcbsp_id].semlock);
	FN_OUT(0);
	return (0);
 ...
 *
 */
int omap2_mcbsp_txmultich_cfg(u32 mcbsp_id, u8 part_mode, u8 part_ablk,
			      u8 part_bblk, u8 parta_enable, u8 partb_enable)
{
	u32 base;
	int ret;
	FN_IN;
	...
	/* check for valid partition blocks */
	if (part_ablk > OMAP2_MCBSP_MULTICH_BLK3 ||
	    part_bblk > OMAP2_MCBSP_MULTICH_BLK3)
		goto tx_mch_err;

	/* Max supported multi channels = 128 */
	if (parta_enable > OMAP2_MCBSP_MAX_MULTI_CHS ||
	    partb_enable > OMAP2_MCBSP_MAX_MULTI_CHS)
		goto tx_mch_err;

	OMAP2_MCBSP_WRITE(base, MCR2, 0x00000000);
 ...
 *
 */
int omap2_mcbsp_rxmultich_cfg(u32 mcbsp_id, u8 part_mode, u8 part_ablk,
			      u8 part_bblk, u8 parta_enable, u8 partb_enable)
{
	u32 base;
	int ret;
	...
	OMAP2_MCBSP_WRITE(base, MCR1, 0x00000000);
...
int omap2_mcbsp_set_recv_params(u32 mcbsp_id, omap2_mcbsp_transfer_params * rp)
{
	...
	rx_params.elem_count = 1;
	rx_params.frame_count = 1;
/* If McBSP FIFO is used, do a packet sync DMA */
	...
	rx_params.src_fi = 0;
#endif
	rx_params.trigger = omap2_mcbsp_dma_rx[mcbsp_id];
	rx_params.src_or_dst_synch = 0x01;
	rx_params.src_amode = OMAP_DMA_AMODE_CONSTANT;
	rx_params.src_start = omap2_mcbsp_drr[mcbsp_id];
	rx_params.src_ei = 0x0;
	rx_params.dst_start = 0x0;	/* buffer start address will be provided in receive */
	...
	D2(KERN_INFO "rx DI ei=%d, fi=%d, dt=%d,amode=0x%x\n", rx_params.src_ei,
	   rx_params.src_fi, dt, rx_params.src_amode);
	...
	omap2_mcbsp_info[mcbsp_id].rx_data_type = rx_params.data_type << 0x1;
...
/* IF McBSP FIFO is used, use packet sync DMA*/
	...
	if (omap2_mcbsp_info[mcbsp_id].dma_chain_id_tx == -1) {
		err = omap_request_dma_chain(mcbsp_id,
				   omap2_mcbsp_info[mcbsp_id].name,
				   omap2_mcbsp_txdmacb, &chain_id,
				   omap2_mcbsp_max_dmachs_tx[mcbsp_id],
 ...
 */
static irqreturn_t
omap2_mcbsp_master_isr(int irq, void *interface, struct pt_regs *regs)
{
	...
	if (omap2_mcbsp_info[mcbsp_id].isr != NULL) {
		...
		omap2_mcbsp_info[mcbsp_id].
		    isr(omap2_mcbsp_info[mcbsp_id].irqargs, irqstatus, regs);
	}
