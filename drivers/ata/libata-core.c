 ...
 *
 *	RETURNS:
 *	Device type, %ATA_DEV_ATA, %ATA_DEV_ATAPI, %ATA_DEV_CEATA, or %ATA_DEV_UNKNOWN
...
unsigned int ata_dev_classify(const struct ata_taskfile *tf)
{
	...
	else if (((tf->lbam == 0x14) && (tf->lbah == 0xeb)) ||
 ...
 retry:
	...
	if (err_mask) {
		...
		if (may_fallback &&
			...
			if (class == ATA_DEV_ATA || class == ATA_DEV_CEATA)
				class = ATA_DEV_ATAPI;
			else
				class = ATA_DEV_ATA;
			goto retry;
