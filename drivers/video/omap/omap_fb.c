...
static int omap24xxfb_rotation = -1;	// -1 = no rotation support
static int omap24xxfb_mirroring = 0;	// the status of mirroring
...
extern int get_panel_var(struct fb_var_screeninfo *o_p_fb_var, struct omap_fb_panel_select *i_p_sel);
static int omap24xxfb_init_g6_disp_out(struct fb_var_screeninfo * i_p_fb_var);
static void omap24xxfb_cleanup_g6_disp_out(void);
extern int set_panel_output(struct omap_fb_panel_select *i_p_sel);
#endif
...
int omap24xxfb_set_output_layer(int layer);
 ...
 */
#define omap24xxfb_gfx_clk_period() (1000000000UL/(omap2_disp_get_dssfclk()/1000))
 ...
 */
#ifdef CONFIG_MACH_ARCHOS_G6
// frame buffer size WVGA for LCD 800*480 ARGB gfx vid1 vid2
// frame buffer size for hdready out 1280*720
// allocate only one fb for now, maybe 2 later
#define omap24xxfb_fb_size(rotation) \
	rotation ? (2048 * 480 * (32/8)) : (1280 * 720 * (32/8) * NB_FRAME_BUFFERS)
//	rotation ? (2048 * 480 * (16/8)) : (1280 * 720 * (32/8) * 3)

//	rotation ? (2048 * 480 * (24/8)) : (800 * 480 * (32/8) * 3)
 ...
 */
#ifdef CONFIG_MACH_ARCHOS_G6
#define omap24xxfb_vrfb_size()	(MAX_PIXELS_PER_LINE * 720 * (32/8))
#else
		      ...
		      struct fb_var_screeninfo *var)
{
	...
	if (oinfo->rotation_support) {
		...
		if (!((var->bits_per_pixel == 8) ||
			...
			printk(KERN_INFO "bits_per_pixel == 8 or 16\n");
			omap2_disp_put_dss();
			return -EINVAL;
		}
		...
		default:
			if ((var->xres > display_xres) ||
				...
				printk(KERN_INFO "rotate 0 180 xres > display_xres or yres > display_yres\n");
				omap2_disp_put_dss();
				return -EINVAL;
			}
			break;
		...
		case 270:
			if ((var->xres > display_yres) ||
				...
				printk(KERN_INFO "rotate 90 270 xres > display_xres or yres > display_yres\n");
				omap2_disp_put_dss();
				return -EINVAL;
		}
			break;
		...
		if ((var->xres > display_xres) ||
		    (var->yres > display_yres)) {
			printk(KERN_INFO "xres = %d > display_xres = %d, or yres=%d > display_yres=%d\n",
			var->xres, display_xres, var->yres, display_yres );
			omap2_disp_put_dss();
			return -EINVAL;
		}
	}
	...
	if (pixclock < omap24xx_display_pixclock_min(output_dev)){
		printk(KERN_INFO "pixclock %d< omap24xx_display_pixclock_min %d\n",pixclock,omap24xx_display_pixclock_min(output_dev));
		omap2_disp_put_dss();
		return -EINVAL;
	}
	...
	if (pixclock > omap24xx_display_pixclock_max(output_dev)) {
		printk(KERN_INFO "pixclock %d > omap24xx_display_pixclock_max %d\n",pixclock,omap24xx_display_pixclock_max(output_dev));
		omap2_disp_put_dss();
		return -EINVAL;
	}
...
omap24xxfb_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	if (omap24xxfb_check_mode(oinfo, &v)){
		printk(KERN_INFO "omap24xxfb_check_mode ko.\n");
		omap2_disp_put_dss();
		return -EINVAL;
	}
	...
	switch (v.bits_per_pixel) {
		...
		case 17 ... 32:
			...
			break;
		default:
			printk(KERN_INFO "v.bits_per_pixel ko.\n");
			omap2_disp_put_dss();
			return -EINVAL;
	}
	...
	switch (var_to_depth(&v)) {
		...
		case 32:
			...
			v.green.offset = 8;
			v.blue.offset = 0;
			v.red.length = v.blue.length = v.green.length = v.transp.length = 8;
			break;
		default:
			printk(KERN_INFO "var_to_depth ko.\n");
			omap2_disp_put_dss();
			return -EINVAL;
	}
	...
	if (display_xres > MAX_PIXELS_PER_LINE ||
		...
		printk(KERN_INFO "panel size is too big (max= %dx%d).\n",MAX_PIXELS_PER_LINE,MAX_LINES);
		omap2_disp_put_dss();
		return -EINVAL;
	}

	if (display_xres == 0 || display_yres == 0) {
		printk(KERN_INFO "panel size is 0 x 0.\n");
		omap2_disp_put_dss();
		return -EINVAL;
	}
	if (v.xres_virtual < v.xres || v.yres_virtual < v.yres) {
		printk(KERN_INFO "resolution %dx%d is biger than the framebuffer %dx%d\n",v.xres,v.yres,v.xres_virtual,v.yres_virtual);
		omap2_disp_put_dss();
		return -EINVAL;
	}
 	...
 	if (!oinfo->rotation_support) {
		if (display_xres < v.xres || display_yres < v.yres) {
			printk(KERN_INFO "panel size %dx%d is smaller than the resolution %dx%d\n",display_xres,display_yres,v.xres,v.yres);
			omap2_disp_put_dss();
			return -EINVAL;
		}
	} else {
		...
		default:
			if (display_xres < v.xres ||
			    display_yres < v.yres) {
				printk(KERN_INFO "rotate 0 180 error\n");
				omap2_disp_put_dss();
				return -EINVAL;
			}
			break;
		...
		case 270:
			if (display_xres < v.yres ||
			    display_yres < v.xres) {
				printk(KERN_INFO "rotate 90 270 error\n");
				omap2_disp_put_dss();
				return -EINVAL;
		}
			break;
	...
	if (v.xoffset > v.xres_virtual - v.xres) {
		printk(KERN_INFO "graphic x offset is out of framebuffer width\n");
		omap2_disp_put_dss();
		return -EINVAL;
	}
	if (v.yoffset > v.yres_virtual - v.yres) {
		printk(KERN_INFO "graphic y offset is out of framebuffer height\n");
		omap2_disp_put_dss();
		return -EINVAL;
	}
	...
	{
		printk(KERN_INFO "bits_per_pixel ko\n");
		omap2_disp_put_dss();
		return -EINVAL;
	}
	...
	{
		printk(KERN_INFO "not enough memory to support this framebuffer.\n");
		omap2_disp_put_dss();
		return -EINVAL;
	}
			  ...
			  int rotation, int mirroring)
{
	...
	if ((rotation > 0) && (rotation % 90 != 0))
		...
		if (rotation < 0) {
			...
			printk(KERN_INFO "omap24xxfb_set_dma_params:view_address_base=%#x start=%#x, total_width=%d,img_width=%d\n",
			       view_address_base,start,total_width,img_width);
		}
		else {
...
omap24xxfb_set_par(struct fb_info *info)
{
	...
	omap2_disp_get_dss();
#ifdef CONFIG_MACH_ARCHOS_G6
	omap2_disp_set_clkrate_g6(omap2_disp_get_clkrate_g6_from_pixclock(PICOS2KHZ(var->pixclock)));	
#endif
...
static int omap24xxfb_set_par_light(struct fb_info *info)
{
	struct omap24xxfb_info *oinfo = (struct omap24xxfb_info *) info->par;
	struct fb_var_screeninfo *var = &info->var;
	u32 clkdiv, hbp, hfp, hsw, vbp, vfp, vsw;
...
void omap24xxfb_get_vsync( int *rate, int *scale )
{
	...
	const struct vrs_t {
		...
		{ 31, 3336, 200 },	// TV cradle NTSC
		{ 78, 3336, 200 },	// MiniDock  NTSC
		{ 25, 4000, 200 },	// TV cradle PAL
		{ 65, 4000, 200 },	// MiniDock  PAL
		{ 49, 2010, 100 },	// LCD
		{ 50, 2000, 100 },	// HDMI 50Hz
		{ 60, 1667, 100 },	// HDMI 60Hz
	};
	
	int i;
	for( i = 0; i < sizeof( vsr ) / sizeof( struct vrs_t ); i++ ) {
		if( vsr[i].vsync == oinfo->vsync ) {
			*rate  = vsr[i].rate;
			*scale = vsr[i].scale;
			return;
		}
...
omap24xxfb_set_output_layer(int layer)
{
	...
	pix.pixelformat = V4L2_PIX_FMT_RGB24/*V4L2_PIX_FMT_RGB565*/;
	...
	if (layer == OMAP2_GRAPHICS) {
		...
		omap2_disp_reg_sync( omap2_disp_get_output_dev( layer ));
		wait_for_reg_sync(SCHEDULE_WAIT, oinfo->timeout);
	}
...
omap24xxfb_blank(int blank_mode, struct fb_info *info)
{
	...
	if ((blank_mode) && (!oinfo->blanked)) {
		...
		omap2_disp_enable_output_dev(output_dev);
		wait_for_reg_sync(SCHEDULE_WAIT, oinfo->timeout);
		oinfo->blanked = 0;
		enable_backlight();
	}
...
{
	struct omap24xxfb_info *oinfo = (struct omap24xxfb_info *) info->par;/*device dependent info*/
	void __user *argp = (void __user *)arg;
	int mirroring;
	int ret = 0;
	int enable = 0;
	struct omap_lcd_info lcd_inf;
	struct omap_fb_panel_select panel_select;
	...
	case FBIO_LCD_PUT_SCREENINFO:
		if (!copy_from_user(&lcd_inf, argp, sizeof(lcd_inf))) {
			omap2_disp_get_dss();
...
#endif
#ifdef CONFIG_MACH_ARCHOS_G6
	/* This ioctl accepts a structure of type omap_fb_panel_select and
	* uses that to configure video output (lcd, hdmi, tv) and the format (vga, 480p, pal, ntsc ...)
	*/
	case FBIO_SET_PANEL_OUTPUT:
		if (!copy_from_user(&panel_select, argp, sizeof(struct omap_fb_panel_select))) {
// 			printk(KERN_INFO "FBIO_SET_PANEL_OUTPUT itf=%d, format=%d\n",
// 			       panel_select.interface_id,panel_select.format_id);

			/* NOTE:
			 * this code assumes disp_usage==1, because
			 * omap24xxfb_cleanup_g6_disp_out will put_dss() and
			 * switch off the clocks. omap24xxfb_init_g6_disp_out
			 * will initialize the display with new values and do
			 * get_dss() only to have disp_usage==1 again.
			 */
			
			/* get the framebuffer config associated to the selected panel */
			if (get_panel_var(&var_screeninfo,&panel_select) == 0) {
				omap24xxfb_cleanup_g6_disp_out(); /* does omap2_disp_put_dss() */
				
				/* really select the panel and apply new timings */
				if (set_panel_output(&panel_select) != 0) {
					printk(KERN_ERR "set_panel_output error\n");
					ret = -EINVAL;
				} else {
					omap24xxfb_init_g6_disp_out(&var_screeninfo);	
				}
			} else {
				printk(KERN_ERR "get_panel_var error\n");
				ret = -EINVAL;
			}
			return ret;
		}
	...
	case FBIO_GFX_LAYER_ENABLE:
		if (!copy_from_user(&enable, argp, sizeof(enable))) {
//			printk(KERN_INFO "FBIO_GFX_LAYER_ENABLE %d \n", enable );

			/* this doesn't make sense if the display is blanked */
			if (oinfo->asleep || oinfo->blanked)
				return -EINVAL;
			
			/* wait for shadow registers to be updated before trying to change them */	
			wait_for_reg_sync(SCHEDULE_WAIT, oinfo->timeout);
			...
			/* make sure to wait 'til the shadow registers are updated */
			wait_for_reg_sync(SCHEDULE_WAIT, oinfo->timeout);
			return ret;
		}
		break;

	case FBIO_OUTPUT_ENABLE:
		if (!copy_from_user(&enable, argp, sizeof(enable))) {
//			printk(KERN_INFO "FBIO_OUTPUT_ENABLE %d \n", enable );

			/* if the display is off already this doesn't make sense */
			if (oinfo->asleep || oinfo->blanked) {
//				printk("display is off!! %d %d\n", oinfo->asleep, oinfo->blanked );				
				return -EINVAL;
			}
...
static int omap24xxfb_init_g6_disp_out( struct fb_var_screeninfo *i_p_fb_var )
{
	...
	/*NOTE SL no rotation support */
	oinfo->rotation_support = 0;
	...
	/* NOTE SL : store new var screeninfo */
	memcpy( &( info->var ), i_p_fb_var, sizeof( struct fb_var_screeninfo ) );

	/*NOTE SL : simplified omap24xxfb_set_par(info) */
	if ( !oinfo->asleep ) {
		info->fix.line_length = ( i_p_fb_var->xres_virtual * i_p_fb_var->bits_per_pixel ) / 8;
		info->fix.visual      = ( ( var_to_depth( i_p_fb_var ) <= 8 ) ? FB_VISUAL_PSEUDOCOLOR : FB_VISUAL_TRUECOLOR );

		/* Graphics window parameters NOTE : no rotation */
		omap2_disp_config_gfxlayer( i_p_fb_var->xres, i_p_fb_var->yres, var_to_depth( i_p_fb_var ) );
		omap2_disp_set_gfx_palette( oinfo->palette_phys );
		
		/* Graphics DMA parameters */
		omap24xxfb_set_dma_params( i_p_fb_var, &info->fix, -1 /*no rotation */ , omap24xxfb_mirroring );

		oinfo->hsync = horizontal_sync_freq( i_p_fb_var );
		oinfo->vsync = vertical_sync_freq  ( i_p_fb_var );
		...
		omap2_disp_enable_output_dev( omap2_disp_get_output_dev(OMAP2_GRAPHICS) );
	...
	printk( FB_NAME ": display mode %dx%dx%d", info->var.xres, info->var.yres, var_to_depth( &info->var ) );
	printk( " hsync %dHz  vsync %dHz", oinfo->hsync, oinfo->vsync );
...
static void omap24xxfb_cleanup_g6_disp_out(void)
{
	...
	printk(KERN_DEBUG "framebuffer config removed\n");
}
 ...
 *  Initialization
 */
static int __init omap24xxfb_init(void)
{
	struct omap24xxfb_info *oinfo;
	struct fb_info *info;
	int output_dev = omap2_disp_get_output_dev(OMAP2_GRAPHICS);
	...
	/* the clocks are still on from early display init */
	omap2_disp_get_dss();
	...
	{
		// copy banner in new fb address
		int j;

		unsigned char *src, *psrc;
		unsigned char *dest = (unsigned char*)oinfo->fb_base;
		
		src = psrc = (unsigned char*)ioremap(0x83700000, oinfo->fb_size);

		printk(KERN_DEBUG " dest %p, src %p \n", dest, src);

		for (j=0; j < 480; j++) {
			memcpy( dest, psrc, 3200 );
			dest += 3200;
			psrc += 3200;
		}
		iounmap((void*)src);
...
// 	omap2_dss_rgb_disable();	
	...
	/* give up dss clocks only after we don't use DSS anymore */
	omap2_disp_put_dss();
...
EXPORT_SYMBOL(omap24xxfb_get_vsync);
