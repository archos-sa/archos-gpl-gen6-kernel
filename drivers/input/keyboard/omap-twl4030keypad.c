...
static char omap_kp_spurious(unsigned char key_state)
{
	unsigned char i;

	for (i = 0; key_state!=0; key_state >>=1)
		if(key_state & 01)
			++i;
...
static void twl4030_kp_scan(int release_all)
{
	...
	for (row = 0; row < NUM_ROWS; row++) {
		...
		if (omap_kp_spurious(new_state[row]))
			return;
		...
		for (col = 0; col < NUM_COLS; col++) {
			...
			dev_dbg(dbg_dev, "key %s %s (%u,%u)\n", switch_name[row][col],
				(new_state[row] & (1 << col)) ?
				"press" : "release", row, col);
