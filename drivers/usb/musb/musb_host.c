...
struct urb *next_urb(struct musb_qh *qh)
{
	...
	if( !is_host_enabled(qh->hw_ep->musb) )
		return NULL;
...
/* FIXME if !bEnd, do the same for RX ... */
		...
		if (can_bulk_split(pThis, qh->type))
			...
			wLoadCount = min((u32) qh->maxpacket, dwLength);
		...
		if (usb_pipeisoc(nPipe)) {
			...
			if (!bDone && dma) {
				...
				wTxCsrVal &= ~(MGC_M_TXCSR_AUTOSET
					| MGC_M_TXCSR_DMAMODE
					| MGC_M_TXCSR_DMAENAB);
				wTxCsrVal |= MGC_M_TXCSR_MODE;
				musb_writew(epio, MGC_O_HDRC_TXCSR,
					wTxCsrVal | MGC_M_TXCSR_MODE);
	
				qh->segsize = min(d->length, dma->dwMaxLength);
	
				dma->bDesiredMode = 0;
				wTxCsrVal &= ~(MGC_M_TXCSR_AUTOSET
						| MGC_M_TXCSR_DMAMODE);
				wTxCsrVal |= (MGC_M_TXCSR_DMAENAB);
	
				musb_writew(epio, MGC_O_HDRC_TXCSR, wTxCsrVal);
	
				dma_ok = pThis->pDmaController->
					 channel_program(
						dma, qh->maxpacket,
						dma->bDesiredMode,
						pUrb->transfer_dma + d->offset,
						qh->segsize);
				if (dma_ok) return;
				if (!dma_ok) {
					pThis->pDmaController->
						channel_release(dma);
					dma = pEnd->tx_channel = NULL;
					wTxCsrVal &= ~(MGC_M_TXCSR_AUTOSET
						  | MGC_M_TXCSR_DMAMODE
						  | MGC_M_TXCSR_DMAENAB);
					musb_writew(epio, MGC_O_HDRC_TXCSR,
...
/* Nuances of Host side RX (IN) using mode 1:
 ...
 *
 * 	Therefore, mode 1 is basically usable only for mass storage class; 
 *	essentially all other protocols terminate transfers on short packets.
 *
 * REVISIT
...
void musb_host_rx(struct musb *pThis, u8 bEnd)
{
	...
	if (dma && (wRxCsrVal & MGC_M_RXCSR_DMAENAB)) {
		...
		if (usb_pipeisoc(nPipe)) {
			...
			/* bDone if pUrb buffer is full or short packet recd */
		...
		if (dma) {
			...
			if (usb_pipeisoc(nPipe)) {
				...
				if (wRxCount > d->length) {
					...
					length = wRxCount;
				d->status = status;
				pBuffer = pUrb->transfer_dma + d->offset;
			} else {
				length = wRxCount;
				pBuffer = pUrb->transfer_dma +
						pUrb->actual_length;
			}

			dma->bDesiredMode = 0;
#ifdef USE_MODE1
			/* because of the issue described above, mode 1 will
			...
			if ((pUrb->transfer_flags &
				...
				&& (pUrb->transfer_buffer_length -
					...
					> qh->maxpacket)
				dma->bDesiredMode = 1;
			if (wRxCount < pEnd->wMaxPacketSizeRx) {
				length = wRxCount;
				dma->bDesiredMode = 0;
			} else {
 ...
 */
			...
			if (dma->bDesiredMode == 0)
				...
				wVal |= MGC_M_RXCSR_H_AUTOREQ;
#if 0
				musb_writew(pBase, MGC_M_RQPKTCOUNT(bEnd), 
					(length/512 - 1));
#endif
			}
			 ...
			 * adjusted first...
			 */
			status = c->channel_program(dma, qh->maxpacket,
					dma->bDesiredMode, pBuffer, length);
			if (!status) {
...
static int musb_urb_dequeue(struct usb_hcd *hcd, struct urb *urb)
{
	...
	if (status < 0 || (sched && qh != first_qh(sched))) {
		...
		if (usb_pipeisoc(type))
			musb->periodic[qh->hw_ep->bLocalEnd] = NULL;
	} else {
