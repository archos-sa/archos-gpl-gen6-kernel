...
static void musb_gadget_fifo_clear(struct usb_ep *ep)
{
	...
	if (musb_ep->is_in) {
		...
		musb_writew(pBase, MGC_O_HDRC_TXCSR, MGC_M_TXCSR_TXPKTRDY);
		musb_writew(pBase, MGC_O_HDRC_TXCSR, MGC_M_TXCSR_TXPKTRDY);

		musb_writew(pBase, MGC_O_HDRC_TXCSR, MGC_M_TXCSR_FLUSHFIFO);
		musb_writew(pBase, MGC_O_HDRC_TXCSR, MGC_M_TXCSR_FLUSHFIFO);
	} else {
		/* Twice in case of double buffering */
		musb_writew(pBase, MGC_O_HDRC_RXCSR, MGC_M_RXCSR_RXPKTRDY);
		musb_writew(pBase, MGC_O_HDRC_RXCSR, MGC_M_RXCSR_RXPKTRDY);

		musb_writew(pBase, MGC_O_HDRC_RXCSR, MGC_M_RXCSR_FLUSHFIFO);
		musb_writew(pBase, MGC_O_HDRC_RXCSR, MGC_M_RXCSR_FLUSHFIFO);
	}
...
static int musb_gadget_has_data(struct usb_ep *ep)
{
	...
	wCsr = musb_readw(pBase, MGC_O_HDRC_RXCSR);
	if ((wCsr & MGC_M_RXCSR_RXPKTRDY) || !list_empty(&(musb_ep->req_list)))
		return 1;
	else
		return 0;
}
...
int usb_gadget_register_driver(struct usb_gadget_driver *driver)
{
	...
	if (retval == 0) {
		...
		if (musb_gadget_power_constraint)
			constraint_set(musb_gadget_power_constraint, CO_LATENCY_MPUOFF_COREON);
