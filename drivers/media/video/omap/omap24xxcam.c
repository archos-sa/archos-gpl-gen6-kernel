	...
	enum v4l2_field field)
{
	...
	if (!err) {
		...
		if (vb->baddr) {
			...
			consistent_sync(vb->dma.vmalloc, vb->dma.nr_pages << PAGE_SHIFT,
					vb->dma.direction);
		}
	} else
		omap24xxcam_vbq_release(q, vb);
...
omap24xxcam_vbq_queue(struct videobuf_queue *q, struct videobuf_buffer *vb)
{
	...
	/* Videobuf buffers are aligned to pagesize.
 	 ...
 	 * unnecessary reading
	 */
	if ((vb->size + vb->dma.sglist[0].offset) % PAGE_SIZE) {
		vb->dma.sglist[vb->dma.sglen-1].length = 
			(vb->size + vb->dma.sglist[0].offset) % PAGE_SIZE;
	}   
...
#ifdef CONFIG_PM
static int
omap24xxcam_suspend(struct omap_dev *dev, u32 state)
{
...
static int
omap24xxcam_resume(struct omap_dev *dev)
{
	struct omap24xxcam_device *cam = omap_get_drvdata(dev);
...
static int
omap24xxcam_probe(struct omap_dev *dev)
{
	return 0;
}
static void
