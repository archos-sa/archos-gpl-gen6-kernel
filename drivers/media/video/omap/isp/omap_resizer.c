...
int rsz_start(struct rsz_resize *resize, struct channel_config *rsz_conf_chan)
{
	...
	ret =
	    isp_set_callback(CBK_RESZ_DONE, rsz_isr, (void *)NULL,
			     (void *)NULL);
	if (ret) {
	...
	ret = isp_unset_callback(CBK_RESZ_DONE);
		   ...
		   struct channel_config *rsz_conf_chan)
{
	...
	rsz_conf_chan->register_config.rsz_sdr_inoff =
	    ((params->in_pitch) & ISPRSZ_SDR_INOFF_OFFSET_MASK);
	...
	if (hrsz >= UP_RSZ_RATIO && hrsz <= DOWN_RSZ_RATIO) {
		...
		printk("horizontal scaling ratio invalid: %d, %d, %d\n",
		       hrsz, params->in_hsize, params->out_hsize);
		return -EINVAL;
	}
	if (vrsz < UP_RSZ_RATIO || vrsz > DOWN_RSZ_RATIO1) {
		printk("vertical scaling ratio invalid:%d,%d,%d\n",
		       vrsz, params->in_vsize, params->out_vsize);
		return -EINVAL;
	}
	rsz_conf_chan->register_config.rsz_out_size =
	    (params->out_hsize & ISPRSZ_OUT_SIZE_HORZ_MASK);

	rsz_conf_chan->register_config.rsz_out_size |=
	    ((params->out_vsize << ISPRSZ_OUT_SIZE_VERT_SHIFT) &
	     ISPRSZ_OUT_SIZE_VERT_MASK);

	if ((params->out_pitch) % ALIGN32) {
		printk("Inavlid	output pitch \n");
		return -EINVAL;
	}
	rsz_conf_chan->register_config.rsz_sdr_outoff =
	    params->out_pitch & ISPRSZ_SDR_OUTOFF_OFFSET_MASK;
	...
	rsz_conf_chan->register_config.rsz_cnt |=
	    ((params->hstph << ISPRSZ_CNT_HSTPH_SHIFT) & ISPRSZ_CNT_HSTPH_MASK);
	...
	rsz_conf_chan->register_config.rsz_cnt |=
	    ((params->vstph << ISPRSZ_CNT_VSTPH_SHIFT) & ISPRSZ_CNT_VSTPH_MASK);
	...
	for (coeffcounter = ZERO; coeffcounter < MAX_COEF_COUNTER;
		...
		rsz_conf_chan->register_config.
		    ...
		    (params->
		     vfilt_coeffs[2 *
				  coeffcounter] & ISPRSZ_VFILT10_COEF0_MASK);
		...
		rsz_conf_chan->register_config.
		    ...
		    ((params->
		      vfilt_coeffs[2 * coeffcounter +
				   NEXT] << ISPRSZ_VFILT10_COEF1_SHIFT) &
	...
	rsz_conf_chan->register_config.rsz_cnt |=
	    ((hrsz - 1) & ISPRSZ_CNT_HRSZ_MASK);

	/* Configuring the vertical     ratio */
	rsz_conf_chan->register_config.rsz_cnt |=
	    (((vrsz - 1) << ISPRSZ_CNT_VRSZ_SHIFT) & ISPRSZ_CNT_VRSZ_MASK);
