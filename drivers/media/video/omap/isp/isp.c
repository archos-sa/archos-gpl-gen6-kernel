...
#define PM_PREPWSTST_CAM  	__REG32(0x48306FE8)

//#define ISP_DEBUG_IRQ
#if defined ISP_DEBUG_IRQ
	static int isp_irq_cnt =0;
	module_param(isp_irq_cnt, int, S_IRUGO|S_IWUSR);

	#define _inc_irq_cnt() (isp_irq_cnt++)
#else
...
/* from isph3a.c */
extern int  isph3a_aewb_init(void);
extern void isph3a_aewb_cleanup(void);
/* from isphist.c */
extern int isp_hist_init(void);
extern void isp_hist_cleanup(void);
/* from isppreview.c */
extern int isp_preview_init(void);
extern void isp_preview_cleanup(void);
/* from ispresizer.c */
extern int isp_resizer_init(void);
extern void isp_resizer_cleanup(void);
/* from ispmmu.c */
extern int ispmmu_init(void);
extern void ispmmu_cleanup(void);
/* from ispccdc.c */
extern int isp_ccdc_init(void);
extern void isp_ccdc_cleanup(void);

/*Structure for IRQ related info */
static struct ispirq {
	isp_callback_t isp_callbk[11];
	void *isp_callbk_arg1[11];
	void *isp_callbk_arg2[11];
} ispirq_obj;
		 ...
		 void *arg1, void *arg2)
{
	...
	case CBK_CCDC_VD0:
		...
		break;
	case CBK_HS_VS:	
		omap_writel(omap_readl(ISP_IRQ0STATUS)|IRQ0ENABLE_HS_VS_IRQ, ISP_IRQ0STATUS);
		omap_writel(omap_readl(ISP_IRQ0ENABLE)|IRQ0ENABLE_HS_VS_IRQ,
				ISP_IRQ0ENABLE);
		break;
	case CBK_SBL_OVF:	
		DPRINTK_ISPCTRL("enable SBL OVF irq\n");
		omap_writel(omap_readl(ISP_IRQ0STATUS)|IRQ0ENABLE_OVF_IRQ, ISP_IRQ0STATUS);
		omap_writel(omap_readl(ISP_IRQ0ENABLE)|IRQ0ENABLE_OVF_IRQ, ISP_IRQ0ENABLE);
		break;

	case CBK_OCP:	
		DPRINTK_ISPCTRL("enable OCP ERR irq\n");
		omap_writel(omap_readl(ISP_IRQ0STATUS)|IRQ0ENABLE_OCP_ERR_IRQ, ISP_IRQ0STATUS);
		omap_writel(omap_readl(ISP_IRQ0ENABLE)|IRQ0ENABLE_OCP_ERR_IRQ, ISP_IRQ0ENABLE);
		break;
	case CBK_BUFF:	
		DPRINTK_ISPCTRL("enable CBUFF ERR irq\n");
		omap_writel(omap_readl(ISP_IRQ0STATUS)|IRQ0ENABLE_CBUFF_IRQ, ISP_IRQ0STATUS);
		omap_writel(omap_readl(ISP_IRQ0ENABLE)|IRQ0ENABLE_CBUFF_IRQ, ISP_IRQ0ENABLE);
		break;
#if 0
	case CBK_CCDC_VD1:
...
{
	/* Now use fixed clock until frame rate control*/
	omap_writel((omap_readl(ISP_TCTRL_CTRL))
			|(ISP_XCLKA_DEFAULT<<ISPTCTRL_CTRL_DIVA_SHIFT)
			, ISP_TCTRL_CTRL);
	return xclk;
}
...
omap34xx_isp_isr(int irq, void *ispirq_disp)
{
	...
	*/
	...
	/* if an ISR is registered for this interface, execute the same */
		...
		if ((irqstatus & SBL_OVF) == SBL_OVF)   
			if (irqdis->isp_callbk[8])
			  irqdis->isp_callbk[8](SBL_OVF,
						irqdis->isp_callbk_arg1[8],
						irqdis->isp_callbk_arg2[8]);
		
		if ((irqstatus & OCP_ERR) == OCP_ERR)   
			if (irqdis->isp_callbk[CBK_OCP])
			  irqdis->isp_callbk[CBK_OCP](OCP_ERR,
						irqdis->isp_callbk_arg1[CBK_OCP],
						irqdis->isp_callbk_arg2[CBK_OCP]);		

		if ((irqstatus & CBUFF_ERR) == CBUFF_ERR)   
			if (irqdis->isp_callbk[CBK_BUFF])
			  irqdis->isp_callbk[CBK_BUFF](CBUFF_ERR,
						irqdis->isp_callbk_arg1[CBK_BUFF],
						irqdis->isp_callbk_arg2[CBK_BUFF]);		
	}
...
isp_get(void)
{
	...
	if (isp_obj.ref_count == 0) {
		...
		/* Context restore */
		if (off_mode == 1 && PM_PREPWSTST_CAM == 0x0 ) {

			isp_restore_context(isp_reg_list);
#ifdef VIDEO_OMAP34XX_CAMERA
