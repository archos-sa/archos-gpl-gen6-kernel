...
int ispccdc_get_field(void)
{
	return (omap_readl(ISPCCDC_SYN_MODE) & ISPCCDC_SYN_MODE_FLDSTAT) == ISPCCDC_SYN_MODE_FLDSTAT;
}
				...
				enum ccdc_output output)
{
	...
	struct ispccdc_syncif syncif = { 0 };
	...
	*/
	...
	case CCDC_OTHERS_MEM:
		...
		syn_mode &= ~ISPCCDC_SYN_MODE_EXWEN;
		if (input != CCDC_YUV_BT) // not need for tvp5146 input
		{
...
ispccdc_config_sync_if(struct ispccdc_syncif syncif)
{
	...
	if(!(syncif.bt_r656_en)) {
		...
		/* data size must be configured via CCDC_CFG */
		if (syncif.datsz == DAT8) {
			omap_writel(omap_readl(ISPCCDC_CFG)
				& (~ISPCCDC_CFG_BW656), ISPCCDC_CFG);
		} else {
			omap_writel(omap_readl(ISPCCDC_CFG)
				| ISPCCDC_CFG_BW656, ISPCCDC_CFG);
		}
		
		/* pix order must be configured via CCDC_CFG */
		if (!syncif.pix_order) {
			omap_writel(omap_readl(ISPCCDC_CFG)
				& (~ISPCCDC_CFG_Y8POS), ISPCCDC_CFG);
		} else {
			omap_writel(omap_readl(ISPCCDC_CFG)
				| ISPCCDC_CFG_Y8POS, ISPCCDC_CFG);
		}
			...
			u32 output_h)
{
	...
	if (ispccdc_obj.ccdc_outfmt == CCDC_OTHERS_VP) {
		...
		omap_writel((((ispccdc_obj.ccdcout_h - 1) &  ISPCCDC_VDINT_0_MASK)
			    ...
			    | (((ispccdc_obj.ccdcout_h - 1) &  ISPCCDC_VDINT_1_MASK) << ISPCCDC_VDINT_1_SHIFT),
			    ISPCCDC_VDINT);
		...
		omap_writel((((ispccdc_obj.ccdcout_h - 25) &  ISPCCDC_VDINT_0_MASK)
			     << ISPCCDC_VDINT_0_SHIFT)
...
ispccdc_print_status(void)
{
	...
	DPRINTK_ISPCCDC("###CCDC REC656IF=0x%x\n", omap_readl(ISPCCDC_REC656IF));
	...
	DPRINTK_ISPCCDC("###CCDC_VDINT =0x%x\n",omap_readl(ISPCCDC_VDINT));
