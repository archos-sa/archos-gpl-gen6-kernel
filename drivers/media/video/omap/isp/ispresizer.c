...
ispresizer_request()
{
	DPRINTK_ISPRESZ(" ispresizer_request\n");
	...
	if (!(ispres_obj.res_inuse)){
		...
		omap_writel(omap_readl(ISP_CTRL) | ISPCTRL_SBL_WR0_RAM_EN | ISPCTRL_SBL_RD_RAM_EN |
...
ispresizer_free()
{
	DPRINTK_ISPRESZ(" ispresizer_free\n");
...
EXPORT_SYMBOL(isp_resizer_init);
EXPORT_SYMBOL(isp_resizer_cleanup);
