...
#define COARSE_INT_TIME_3MP	1300	/* max: frame length - 1 */
...
board_init(void)
{
	...
	twl4030_i2c_write_u8(TWL4030_MODULE_PM_RECIEVER,
...
static struct mt9p012_pll_settings {
	...
	{5, 2, 3, 134, 10, 1,
	   ...
	   COARSE_INT_TIME_1296, 98, 190},
	/* PLL_1296_30FPS */
	{5, 1, 3, 134, 10, 1,
	   VIDEO_WIDTH_2X_BINN, VIDEO_HEIGHT_2X_BINN,
	   8,   8, 2597, 1949, 0x046C, 1794, 1061, 3360,
	   COARSE_INT_TIME_1296, 98, 150},
	/* PLL_648_15FPS */
	{8, 4, 2, 126,  8, 4,
	   VIDEO_WIDTH_4X_BINN, VIDEO_HEIGHT_4X_BINN,
	   8,   8, 2593, 1945, 0x04FC, 1794,  574, 2712,
	   COARSE_INT_TIME_648, 92, 126},
	/* PLL_648_30FPS */
	{8, 2, 2, 126,  8, 2,
	   VIDEO_WIDTH_4X_BINN, VIDEO_HEIGHT_4X_BINN,
	   8,   8, 2593, 1945, 0x04FC, 1794,  574, 2712,
	   COARSE_INT_TIME_648, 68, 126},
	/* PLL_216_15FPS */
	{8, 4, 2, 126,  8, 4,
...
//	set_current_state(TASK_UNINTERRUPTIBLE);
//	schedule_timeout(msecs_to_jiffies(200));  //time needed?
			...
			struct v4l2_fract *timeperframe, void *priv)
{
	...
	pix_clk_freq = (pll_multiplier * MT9P012_XCLK_HZ) /
			(all_pll_settings[current_pll_video].pre_pll_div
			 * all_pll_settings[current_pll_video].vt_pix_clk_div
			 * all_pll_settings[current_pll_video].vt_sys_clk_div);
	...
	DPRINTK("pix clock %d, vt_pixclk %d, pll_mutl = %d\n", pix_clk_freq, 
	...
	max_exposure_time = (exposure_factor / pix_clk_freq) * 1000;
...
mt9p012sensor_set_exposure_time(int mode, u32 exp_time)
{
	...
	if (mode == VIDEO_MODE) {
		if((exp_time < min_exposure_time) ||
			...
			DPRINTK("Exposure time must be between %d ns",
					min_exposure_time);
			DPRINTK(" and %d ns\n",
					max_exposure_time);
			return -EINVAL;
		...
		coarse_int_time = 
			(((exp_time * (pix_clk_freq / 1000)) / 1000)
		...
		if (current_pll_image == PLL_3MP) {
			if((exp_time < MIN_EXP_TIME_CAPTURE_3MP) ||
				...
				DPRINTK("Exposure time not within "
					"the legal range.\n");
				DPRINTK("Exposure time must be between %d ns",
			...
			coarse_int_time =
				(((exp_time * (DEF_PIX_CLK_HZ_3MP / 1000))
				 / 1000) - FINE_INT_TIME_DEFAULT)
				 / LINE_LENGTH_3MP;
		} else {
			...
			coarse_int_time =
				(((exp_time * (DEF_PIX_CLK_HZ_5MP / 1000))
				 / 1000) - FINE_INT_TIME_DEFAULT)
				 / LINE_LENGTH_5MP;
		}
	} else {
...
#ifdef CONFIG_VIDEO_OMAP_AF_D88
	err = d88_af_init(MT9P012_XCLK_HZ);
	if (err)
