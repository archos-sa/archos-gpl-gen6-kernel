...
//printk("w");
		...
		if (buf->state == STATE_DONE) {
			//printk("%s: size = %d, buf= %x\n", __FUNCTION__, buf->size, ((char*)buf->dma.vmalloc)[0]);
			int i;
unsigned char *p=(char*)buf->dma.vmalloc;
#if 0
unsigned char *p0 = p;
int ok=1;
for (i=0; i<256; i++ ) {
	if ( *p != 0x47 ) { 
		//printk("NOK%d: %02x %02x %02x %02x\n", p-p0, p[0], p[1], p[2], p[3] );
		ok=0;
		break;
	}
	p += 192;
...
//			printk("buff size = %lu addr %x\n", buf->size, buf->dma.vmalloc);
//printk("addr %x\n", (uint32_t)(buf->dma.vmalloc)>>12);

#if 1
			/* because of the hardware limitation of the CCDC we use we have gaps between the TS packets */
			/* Call dvb_dmx_swfilter per TS packet */  
			p=(char*)buf->dma.vmalloc;
			for (i = 0; i < 256; i++) {
				dvb_dmx_swfilter(&dvb->demux, p, 188);
				p += 192;
			}
#else
