...
static int helmet_cam_command( struct i2c_client *client,
...
static int helmet_cam_id = 0;

static int helmet_cam_detect_client( struct i2c_adapter *adapter, int address, 
					int kind )
{
	int err;
	const char *client_name = "HELMET camera";
	...
	new_client->driver = &helmet_cam_driver;
	new_client->flags = 0;
...
static int helmet_cam_attach_adapter( struct i2c_adapter *adapter )
{
	int err;
	if ( adapter->nr == 3 ) {
		if ( ( err = i2c_probe( adapter, &addr_data, helmet_cam_detect_client ) ) ) {
			printk("helmet cam i2c: Adapter attach failed.\n");
		}
		return err;
	} else 
		return 1;
...
static int helmet_cam_detach_client( struct i2c_client *client )
{
	int err;

	if ( ( err = i2c_detach_client( client ) ) ) {
		printk("helmet cam i2c: Client deregistration failed, client not detached.\n");
		return err;
	}
...
static struct i2c_driver helmet_cam_driver = {
	...
	.id		= I2C_DRIVERID_HELMET,
	.attach_adapter = helmet_cam_attach_adapter,
	.detach_client = helmet_cam_detach_client,
	.command = helmet_cam_command,
};
...
static void __exit helmet_cam_exit( void)
{
	...
	printk("helmet cam i2c: Driver module removed.\n");
