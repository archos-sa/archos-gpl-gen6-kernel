...
struct usb_ep_ops {
	...
	void (*fifo_clear)  (struct usb_ep *ep);
	int (*has_data)  (struct usb_ep *ep);
};
...
/**
 * usb_ep_fifo_clear - clears contents of a fifo
 * @ep: the endpoint whose fifo is being cleared.
 */

static inline void
usb_ep_fifo_clear (struct usb_ep *ep)
{
	if (ep->ops->fifo_clear)
		ep->ops->fifo_clear (ep);
}

/**
 * usb_ep_has_data - returns if the endpoint has data
 * @ep: the endpoint whose fifo is being polled.
 */

static inline int
usb_ep_has_data (struct usb_ep *ep)
{
	if (ep->ops->has_data)
		return ep->ops->has_data (ep);
}
