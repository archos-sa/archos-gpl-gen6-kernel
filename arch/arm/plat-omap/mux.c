...
extern struct pin_config * omap34xx_pins;
...
int __init_or_module omap_cfg_reg(const unsigned long index)
{
	...
	if (cfg->mux_reg) {
		...
		if (cpu_class_is_omap2()) {
			...
			if (cpu_is_omap24xx()) {
				reg_orig = omap_readb(cfg->mux_reg);
				omap_writeb((u8)reg, cfg->mux_reg);
				...
				reg_orig = omap_readw(cfg->mux_reg);
				omap_writew((u16)reg, cfg->mux_reg);
			}
