...
void omap_set_dma_params(int lch, const struct omap_dma_channel_params * params)
{
	...
	if (params->read_prio || params->write_prio)
		omap_dma_set_prio_lch(lch, params->read_prio,
				      params->write_prio);
	if (params->src_burst_mode)
		omap_set_dma_src_burst_mode(lch, params->src_burst_mode);
	if (params->dst_burst_mode)
		omap_set_dma_dest_burst_mode(lch, params->dst_burst_mode);
}
		     ...
		     void *data, int *dma_ch_out)
{
	...
	chan->data = data;
	chan->chain_with_irq_disabled = 0;
#ifndef CONFIG_ARCH_OMAP1
	chan->chain_id = -1;
#endif
...
void omap_free_dma(int lch)
{
	...
	dma_chan[lch].chain_with_irq_disabled = 0;
	spin_unlock_irqrestore(&dma_chan_lock, flags);
...
int omap_dma_ch_running(int lch)
{
...
int omap_request_dma_chain(int dev_id, const char *dev_name,
			   ...
			   const struct omap_dma_channel_params *params)
{
	int *channels;
	int i, err;
...
int omap_dma_chain_status(int chain_id)
{
	/* Check for input params */
	if (unlikely((chain_id < 0 || chain_id >= OMAP_LOGICAL_DMA_CH_COUNT))) {
		printk(KERN_ERR "Invalid chain id: %d\n", chain_id);
		return -EINVAL;
	}
...
/**
 * @brief omap_dma_set_interrupt_ch - Set the interrupt value of all
 * ongoing DMA in chain
 * @param chain_id
 * @param irq - interrupt value to be set for all channel except last
 * @param last_irq - interrupt value for the last channel
 *
 * @return - void
 */
void omap_dma_set_interrupt_ch(int chain_id, u16 irq, u16 last_irq)
{
	int *channels;
	u32 i;
	unsigned long flags;
	...
	spin_lock_irqsave(&dma_chan_lock, flags);
	channels = dma_linked_lch[chain_id].linked_dmach_q;
	for (i = 0; i < (dma_linked_lch[chain_id].no_of_lchs_linked - 1); i++) {
		dma_chan[channels[i]].enabled_irqs = irq;
		OMAP_DMA_CSR_REG(channels[i]) = OMAP2_DMA_CSR_CLEAR_MASK;
		OMAP_DMA_CICR_REG(channels[i]) = 0;
		OMAP_DMA_CICR_REG(channels[i]) = irq;
		dma_chan[channels[i]].chain_with_irq_disabled = 1;
	}

	dma_chan[channels[i]].enabled_irqs = last_irq;
	dma_chan[channels[i]].chain_with_irq_disabled = 1;
	spin_unlock_irqrestore(&dma_chan_lock, flags);

	OMAP_DMA_CSR_REG(channels[i]) = OMAP2_DMA_CSR_CLEAR_MASK;
	OMAP_DMA_CICR_REG(channels[i]) = 0;
	OMAP_DMA_CICR_REG(channels[i]) = last_irq;
}
EXPORT_SYMBOL(omap_dma_set_interrupt_ch);

int omap_dma_set_src_burst_mode_ch(int chain_id, enum omap_dma_burst_mode burst_mode)
{
	int *channels;
	u32 i;
	...
	for (i = 0; i < dma_linked_lch[chain_id].no_of_lchs_linked; i++) {
		omap_set_dma_src_burst_mode(channels[i], burst_mode);
	}
	return 0;
}

EXPORT_SYMBOL(omap_dma_set_src_burst_mode_ch);

int omap_dma_set_dest_burst_mode_ch(int chain_id, enum omap_dma_burst_mode burst_mode)
{
	int *channels;
	u32 i;
	...
	for (i = 0; i < dma_linked_lch[chain_id].no_of_lchs_linked; i++) {
		omap_set_dma_dest_burst_mode(channels[i], burst_mode);
	}
	return 0;
}
EXPORT_SYMBOL(omap_dma_set_dest_burst_mode_ch);
#endif
...
static int omap2_dma_handle_ch(int ch)
{
	...
	if (dma_chan[ch].chain_id != -1) {
		if (dma_chan[ch].chain_with_irq_disabled == 0) {
			...
			int chain_id = dma_chan[ch].chain_id, i;
			int *channels;
			
			channels = dma_linked_lch[chain_id].linked_dmach_q;
			for (i = 0; i < dma_linked_lch[chain_id].no_of_lchs_linked; i++) {
				dma_chan[channels[i]].state = DMA_CH_NOTSTARTED;
				dma_chan[channels[i]].chain_with_irq_disabled = 0;
				if (OMAP_DMA_CLNK_CTRL_REG(channels[i]) & (1 << 15))
					dma_chan[dma_chan[channels[i]].next_linked_ch].state =
						    DMA_CH_STARTED;
				if (!OMAP_DMA_CHAIN_QEMPTY(chain_id))
					OMAP_DMA_CHAIN_INCQHEAD(chain_id);
				dma_linked_lch[chain_id].chain_state = DMA_CHAIN_NOTSTARTED;
...
EXPORT_SYMBOL(omap_dma_ch_running);
