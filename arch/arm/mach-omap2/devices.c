...
#if defined(CONFIG_MMC_OMAP2430) || defined(CONFIG_MMC_OMAP3430) || defined(CONFIG_MMC_OMAP3430_MODULE)
...
#define	OMAP_HSMMC_3_BASE	0x480AD000
static struct omap_mmc_conf mmc3_conf;
static struct resource mmc3_resources[] = {
	{
		.start		= OMAP_HSMMC_3_BASE,
		.end		= OMAP_HSMMC_3_BASE + 0x01FC,
		.flags		= IORESOURCE_MEM,
	},
	{
		.start		= INT_34XX_MMC3_IRQ,
		.flags		= IORESOURCE_IRQ,
	},
};
static struct platform_device mmc_omap_device3 = {
	.name		= "hsmmc-omap",
	.id		= 3,
	.dev = {
		.platform_data	= &mmc3_conf,
	},
...
static void __init omap2_3_init_mmc(void)
{
	...
	if (mmc->enabled) {
		mmc3_conf = *mmc;
		(void) platform_device_register(&mmc_omap_device3);
	}
#endif
