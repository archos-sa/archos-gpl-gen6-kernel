/*
 *  linux/arch/arm/mach-omap3/clock.c
 *
...
#else
static char *id_to_name[] = {"iva2", "mpu", "core", "core", "gfx", NULL, "dss",
				"cam", "per", NULL, "neon"};
...
#else
		prcm_vdd = vdd1_rate_table;
#endif
	else
#ifdef CONFIG_OMAP3430_ES2
		prcm_vdd = vdd2_rate_table + MAX_VDD2_OPP;
#else
		prcm_vdd = vdd2_rate_table;
#endif
...
static int __init omap3_clk_arch_init(void)
{
	...
	vdd1_prcm--) {
#else
	for (vdd1_prcm = vdd1_rate_table; vdd1_prcm->speed; vdd1_prcm++) {
#endif
		DPRINTK("%lu\n", vdd1_prcm->speed);
	...
	for (vdd2_prcm = vdd2_rate_table+MAX_VDD2_OPP; vdd2_prcm->speed;
		vdd2_prcm++) {
#else
	for (vdd2_prcm = vdd2_rate_table; vdd2_prcm->speed; vdd2_prcm++) {
#endif
		DPRINTK("%lu\n", vdd2_prcm->speed);
...
#ifdef CONFIG_AUTO_POWER_DOMAIN_CTRL
/**
 * enable_power_domain: Enable the power for specified domain.
 * @clk: Clock belonging to the domain whose power needs to be enabled.
 *
 * Disable the power for the domain which contains specified clock.
 * It is called from clk_disable function. Note that power state of the PER
 * domain becomes ON as soon as the first device in PER domain calls
 * clk_enable().
*/
int enable_power_domain(struct clk *clk)
{
	...
	DPRINTK("%s: pwr_domain_name %s name %s\n", __FUNCTION__,
...
/**
 * disable_power_domain: Disable the power for specified domain.
 * @clk: Clock belonging to the domain whose power needs to be disabled.
 *
 * Disable the power for the domain which contains specified clock.
 * It is called from clk_disable function. Note that power state of the PER
 * domain becomes OFF as soon as last device in PER domain calls clk_disable().
*/
void disable_power_domain(struct clk *clk)
{
	char *resource_name;
	...
	if (clk->flags & POWER_ON_REQUIRED) {
		if (clk->res != NULL) {
			...
			if (ret)
				DPRINTK("Could not release resource handle for"
						"%s\n", resource_name);
