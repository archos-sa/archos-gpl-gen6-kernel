/*
 * linux/arch/arm/mach-omap3/board-omap3evm-hsmmc.c
 *
 ...
 */
#if defined (CONFIG_TWL4030_CORE) && defined (CONFIG_TWL4030_GPIO) 
#include <linux/err.h>
...
int enable_mmc_power(int slot)
{
	...
	if (slot == mmc_slot1) {
		...
		if (is_sil_rev_greater_than(OMAP3430_REV_ES1_0)) {
			ret = twl4030_i2c_read_u8(TWL4030_MODULE_PM_RECIEVER,
				&reg, VSIM_DEV_GRP);
			if (ret != 0) {
				printk(KERN_ERR
		...
		ret = twl4030_i2c_write_u8(TWL4030_MODULE_PM_RECIEVER, P1_DEV_GRP,
				VMMC2_DEV_GRP);
		if (ret != 0) {
			printk(KERN_ERR
		...
		ret = twl4030_i2c_write_u8(TWL4030_MODULE_PM_RECIEVER,VSEL_S2_18V,
				VMMC2_DEDICATED);
		if (ret != 0) {
...
int disable_mmc_power(int slot)
{
	...
	if (slot == mmc_slot1) {
		...
		ret = twl4030_i2c_write_u8(TWL4030_MODULE_PM_RECIEVER, LDO_CLR,
	       			VMMC1_DEV_GRP);
		if (ret != 0) {
			printk(KERN_ERR "Configuring MMC1 dev grp failed\n");
			return ret;
		}
		ret = twl4030_i2c_write_u8(TWL4030_MODULE_PM_RECIEVER,VSEL_S2_CLR,
				VMMC1_DEDICATED);
		if (ret != 0) {
...
int switch_power_mode(int power_mode)
{
	...
	if (power_mode == 0) {
		ret = twl4030_i2c_write_u8(TWL4030_MODULE_PM_RECIEVER, VSEL_18V,
					 	VMMC1_DEDICATED);
		if (ret != 0) {
			printk(KERN_ERR
		...
		ret = twl4030_i2c_write_u8(TWL4030_MODULE_PM_RECIEVER, VSEL_3V,
					 	VMMC1_DEDICATED);
		if (ret != 0) {
			printk(KERN_ERR
			       "Configuring MMC1 dedicated failed %x\n", ret);
