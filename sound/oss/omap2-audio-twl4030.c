...
/* EVM use auxilliary Line in channel */
...
#ifdef CONFIG_MACH_OMAP3_EVM
    case INPUT_AUX_LINE_IN:
        if ((current_input & INPUT_AUX_LINE_IN) == INPUT_AUX_LINE_IN) {
            u8 set_val_l = (unsigned char)
                    ((gain_l * COMPUTE_PRECISION) / MIC_AMP_INCR);
            DPRINTK("anamic gain_l requested %d, set %d\n", gain_l, set_val_l);
            /* ANAMIC_GAIN */
            ret = audio_twl4030_write(REG_ANAMIC_GAIN,
                    set_val_l << BIT_ANAMIC_GAIN_MICAMPL_GAIN);
            if (!ret) {
                twl4030_local.line = WRITE_LEFT_VOLUME(gain_l) |
...
/* EVM has Aux i/p */
#ifdef CONFIG_MACH_OMAP3_EVM
            ret = twl4030_setvolume(INPUT_AUX_LINE_IN,
                        READ_LEFT_VOLUME
                        (twl4030_local.line),
                        READ_RIGHT_VOLUME
                        (twl4030_local.line));
