...
static int twl4030_enable_input(void)
{
	...
	DPRINTK(KERN_INFO "Enable Input 0x%x HS=0x%x MM=0x%x SM=0x%x\n", current_input,
    ...
    opt_ip = (ret &  ~(BIT_OPTION_ATXL1_EN_M | BIT_OPTION_ATXR1_EN_M |
    ...
    ret = audio_twl4030_write(REG_ANAMIC_GAIN, 0x00);
    if (ret) {
        line = __LINE__;
        goto enable_ip_exit;
    }
    ...
    ret = audio_twl4030_write(REG_ATXL1PGA, 0x0C);
    ret = audio_twl4030_write(REG_ATXR1PGA, 0x0C);
    ret = audio_twl4030_write(REG_AVTXL2PGA, 0x0C);
    ret = audio_twl4030_write(REG_AVTXR2PGA, 0x0C);
 ...
 */
static int omap_twl4030_transfer(int mode, void *buffer_phy, u32 size, void *arg)
{
	int ret = 0;
	int restart = 0;
	FN_IN;
 ...
 */
static int omap_twl4030_transfer_stop(int mode)
{
	int ret = 0;
	FN_IN;
 ...
 */
static int omap_twl4030_transfer_posn(int mode)
{
	int ret = 0;
	int fi, ei;
	FN_IN;
 ...
 * @return 0 if success else return value
 */
static int omap_twl4030_transfer_init(int mode)
{
	int ret = 0;
	FN_IN;
