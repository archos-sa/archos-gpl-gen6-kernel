	...
	if (card->power_state != SNDRV_CTL_POWER_D3) {
		if (chip->card->power_state != SNDRV_CTL_POWER_D3) {
			...
			if (omap_audio_state->s[SNDRV_PCM_STREAM_PLAYBACK].active) {
				ret = codec->codec_transfer_stop(SNDRV_PCM_STREAM_PLAYBACK);
				codec->codec_shutdown();
			}
			if (omap_audio_state->s[SNDRV_PCM_STREAM_CAPTURE].active) {
				ret = codec->codec_transfer_stop(SNDRV_PCM_STREAM_CAPTURE);
				codec->codec_shutdown();
			}
		}
	}
...
static int audio_resume(struct platform_device *dev)
{
	...
	if (card->power_state != SNDRV_CTL_POWER_D0) {                          
		if (chip->card->power_state != SNDRV_CTL_POWER_D0) {
			if (omap_audio_state->s[SNDRV_PCM_STREAM_PLAYBACK].active) {
				codec->codec_init();
				ret = codec->codec_transfer_init(SNDRV_PCM_STREAM_PLAYBACK);
			}
			if (omap_audio_state->s[SNDRV_PCM_STREAM_CAPTURE].active) {
				codec->codec_init();
				ret = codec->codec_transfer_init(SNDRV_PCM_STREAM_CAPTURE);
			}
...
static snd_pcm_uframes_t audio_get_dma_pos(struct omap_alsa_stream *s)
{
	...
	ret = codec->codec_transfer_posn(s->stream_id);
 ...
 */
static void audio_process_dma(struct omap_alsa_stream *s)
{
	struct snd_pcm_substream *substream = s->stream;
...
static int snd_omap_alsa_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_STOP:
		...
		codec->codec_transfer_stop(s->stream_id);
		spin_unlock_irqrestore(&s->dma_lock, flags);
		break;
	default:
		err = -EINVAL;
...
static int snd_omap_alsa_prepare(struct snd_pcm_substream * substream)
{
	...
	codec->codec_set_samplerate(runtime->rate);
	state->samplerate = runtime->rate;
	if (runtime->channels == 1) {
		/* Set Mono_mode, with DSP option */
		codec->codec_set_stereomode(0x02, 0x01); 
	}
	else {
		/* Set Stereo_mode, with DSP option */
		codec->codec_set_stereomode(0x01, 0x01);
	} 
	s->period = 0;
	s->periods = 0;
...
static int snd_card_omap_alsa_open(struct snd_pcm_substream * substream)
{
	...
	snd_pcm_hw_constraint_step(runtime, 0, SNDRV_PCM_HW_PARAM_BUFFER_SIZE, 1024);
	snd_pcm_hw_constraint_step(runtime, 0, SNDRV_PCM_HW_PARAM_PERIOD_SIZE, 1024);
	
	if ((err = codec->codec_transfer_init(stream_id))) {
